...
def run_navigator():
    ...
    branches = list(BRANCH_SYLLABUS.keys())
    print(branches)
    ...
    try:
        ...
        for i, b in enumerate(branches,1):
            print(f"{i}. {b}")
        b_choice = int(input("\nEnter choice number: "))
        selected_branch = branches[b_choice - 1]
    ...
    valid_s = year_m.get(b_year,[])

    if b_year=="1-1":
         print(f"Year {b_year} is starting of btech you try next sem")
         return     


    if not valid_s:
         print(f"Year {b_year} is not yet configured or invalid")
         return

    # if b_year == '2-1': 
    #     valid_s = ["Sem 1", "Sem 2"]
    # elif b_year == '2-2': 
    #     valid_s = ["Sem 1", "Sem 2", "Sem 3"]
    # elif b_year == '3-1': 
    #     valid_s = ["Sem 1", "Sem 2", "Sem 3", "Sem 4"]

    syllabus = BRANCH_SYLLABUS[selected_branch]
    print(syllabus)
    for sem in valid_s:
            if sem in syllabus:
                    print(f"\n--- {sem} ---")
                    for sub in syllabus[sem]:
                        while True:
                            grade = input(f"   Grade for '{sub}': ").upper().strip()
                            if grade in G_P:
                                score = G_P[grade] * 10
                                ...
                                print(f"   Invalid input '{grade}'! Please enter a valid grade (O, A+, A, B+, B, C, P, F).")

            
    final_averages = [sum(pillar_scores[p]) / len(pillar_scores[p]) if pillar_scores[p] else 0 for p in PILLARS]
    feature_df = pd.DataFrame([final_averages], columns=PILLARS.keys())
    if sum(final_averages)==0:
        print(f"Your score is {score} you need to improve and focus on academic")
    elif (sum(final_averages)/len(PILLARS))<30:
         prediction = model.predict(feature_df)[0]
         print(f" PRELIMINARY RECOMMENDATION: {prediction.upper()}")
         print("\n  WARNING: Your scores are quite low. This roadmap may be difficult")
         print("until you streng then your fundamentals.")
    else:    
    
        print("\n",feature_df)
        probs = model.predict_proba(feature_df)
        for career, prob in zip(model.classes_, probs[0]):
        ...
        print(f"\nAI PREDICTED CAREER PATH: {prediction.upper()}")
    
        if prediction in ROADMAPS:
